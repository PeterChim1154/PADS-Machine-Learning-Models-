...
# =====================================================================
# 3. RUN COHORT DISCRIMINATION ENGINE (WITH PCA EMBEDDED)
# =====================================================================
def run_optimized_track(X_all, y_all, groups_all, target_label, control_label):
   ...
   # -------------------------------------------------------------
   # NEW: DIMENSIONALITY REDUCTION & VISUALIZATION LAYER
   # -------------------------------------------------------------
   ...
   # Save the custom PCA visualization plot to disk
   ...
   plt.title(f"PCA Variance Space ({control_label} vs {target_label})\nCaptured Global Variance: {explained_var:.1f}%")
   plt.xlabel("Principal Component 1")
   plt.ylabel("Principal Component 2")
   plt.tight_layout()
  
   filename_clean = f"pca_{control_label.lower().replace(' ', '_')}_vs_{target_label.lower().replace(' ', '_')}.png"
   plt.savefig(filename_clean, dpi=300)
   plt.close()
   print(f" -> Plot successfully exported to working directory: '{filename_clean}'")
   # -------------------------------------------------------------
   ...
   for train_idx, test_idx in sgkf.split(X_tr, y_bin, groups_tr):
       ...
       # Fit Scaling and PCA strictly within validation fold constraints
       scaler = StandardScaler()
       X_train_scaled = scaler.fit_transform(X_train)
       X_test_scaled = scaler.transform(X_test)
      
       # Squeeze down the 80+ processed signal traits into 2 PCA features
       ...
       # Model trains on the 2 low-dimensional PCA parameters
   ...
   print("\n" + "="*50)
   print(f"BALANCED RESULTS TRACK: [{control_label}] vs [{target_label}] (PCA DATA)")
