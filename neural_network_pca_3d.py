...
print("Reverting to Stable Physical-Coherence Normalized Cascade Engine with 3D PCA...")
...
# =====================================================================
# 4. STAGE 2: ADVANCED CASCADED ENSEMBLE (WITH 3D PCA EMBEDDED)
# =====================================================================
def run_normalized_cascade_track(X_all, y_all, groups_all, target_label, control_label):
  ...
  y_bin = (y_tr == target_label).astype(int)
   # -------------------------------------------------------------
  # GENERATE 3D GLOBAL PCA EMBEDDING & PLOT EXPORT
  # -------------------------------------------------------------
  print(f"\nCalculating 3D Dimensionality Reduction for [{control_label}] vs [{target_label}]...")
  ...
  colors = {target_label: "#d95f02", control_label: "#1b9e77"}
  color_vector = [colors[label] for label in y_tr]
 
  ax.scatter(
      X_pca_vis[:, 0], X_pca_vis[:, 1], X_pca_vis[:, 2],
      c=color_vector, alpha=0.6, edgecolors='none', s=20
  )
 
  explained_var = pca_vis.explained_variance_ratio_.sum() * 100
  ax.set_title(f"PyTorch Context - 3D PCA Space ({control_label} vs {target_label})\nCaptured Global Variance: {explained_var:.1f}%")
  ax.set_xlabel("PC 1")
  ax.set_ylabel("PC 2")
  ax.set_zlabel("PC 3")
  ...
  plt.tight_layout()
  filename_clean = f"nn_pca_3d_{control_label.lower()}_vs_{target_label.lower()}.png"
  plt.savefig(filename_clean, dpi=300)
  plt.close()
  print(f" -> 3D Plot exported successfully: '{filename_clean}'")
  # -------------------------------------------------------------
  ...
  patient_true_labels = {}
   ...
   for train_idx, test_idx in sgkf.split(X_tr, y_bin, groups_tr):
      ...
      # Compress variables into 3 dimensions
      ...
      # Configured dynamically for 3 dimensions
      model = DeepWindowRiskAssessor(input_dim=3)
  ...
  true_strings = [target_label if v == 1 else control_label for v in y_meta]
   print("\n" + "="*50)
  print(f"PHYSICAL COHERENCE NET: [{control_label}] vs [{target_label}] (3D PCA DATA)")
