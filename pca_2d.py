...
def plot_2d_track(X_scaled, y, classes_to_keep, title, filename, colors):
    ...
    plt.title(f"{title}\nTotal Extracted Windows: {len(X_subset):,} | Total Variance: {evr.sum()*100:.2f}%", fontsize=12, weight='bold')
    plt.xlabel(f"PC 1 ({evr[0]*100:.1f}%)", fontsize=10)
    plt.ylabel(f"PC 2 ({evr[1]*100:.1f}%)", fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(loc='best', frameon=True, shadow=True)
    plt.tight_layout()
    plt.savefig(filename, dpi=300)
    plt.close()
    print(f"Exported 2D Graph: {filename}")
...
if __name__ == "__main__":
    ...
    if X is not None:
        ...
        plot_2d_track(X_scaled, y, ["PD", "HC"], "Track 1: Idiopathic Baseline Latent Space (PD vs. HC)", "pca_2d_track1_pd_vs_hc.png", palette)
        plot_2d_track(X_scaled, y, ["PD", "DD"], "Track 2: Phenotypic Mimicry Latent Space (PD vs. DD)", "pca_2d_track2_pd_vs_dd.png", palette)
