...
def generate_3d_spin_sequence(X_scaled, y, classes_to_keep, title, file_prefix, colors):
    # Isolate specific cohorts for this track
    mask = np.isin(y, classes_to_keep)
    X_subset = X_scaled[mask]
    y_subset = y[mask]
    
    # Process isolated PCA matrix space
    ...
    # Render underlying scatter distribution
    for cls in classes_to_keep:
        cls_mask = (y_subset == cls)
        # Skip loop if subset has no windows for a targeted class
    ...
    ax.set_xlabel(f"PC 1 ({evr[0]*100:.1f}%)", fontsize=10, fontweight='bold', labelpad=8)
    ax.set_ylabel(f"PC 2 ({evr[1]*100:.1f}%)", fontsize=10, fontweight='bold', labelpad=8)
    ax.set_zlabel(f"PC 3 ({evr[2]*100:.1f}%)", fontsize=10, fontweight='bold', labelpad=8)
    ax.legend(loc='upper right', frameon=True, shadow=True)
    
    # Run loop through 360 degrees split into 10-degree increments
    fixed_elevation = 20
    print(f"\nStarting spin sequence generator for {file_prefix}...")
    
    for azim_angle in range(0, 360, 10):
        ax.view_init(elev=fixed_elevation, azim=azim_angle)
        
        ax.set_title(
            f"{title}\nTotal Windows: {len(X_subset):,} | Variance: {evr.sum()*100:.2f}%\n[Rotation Angle Azimuth: {azim_angle}°]", 
            fontsize=12, weight='bold', pad=10
        )
        
        # Save frame out directly into figs sub-folder
        filename = f"figs/{file_prefix}_angle_{azim_angle:03d}.png"
        plt.savefig(filename, dpi=200, bbox_inches='tight')
        
    plt.close()
    print(f" -> Finished exporting 36 rotation perspectives for {file_prefix}")

if __name__ == "__main__":
    X, y = extract_82d_features()
    if X is not None:
        # Scale whole dataset space globally first
        X_scaled = StandardScaler().fit_transform(X)
        
        # Color palette definition: Orange (PD), Teal (HC), Purple (DD)
        palette = {"PD": "#d95f02", "HC": "#1b9e77", "DD": "#7570b3"}
        
        # Automatically verify or create the figs directory setup
        os.makedirs("figs", exist_ok=True)
        
        # --- LOOP SEQUENCE 1: PD vs Healthy Control ---
        generate_3d_spin_sequence(
            X_scaled, y, ["PD", "HC"], 
            "Track 1: Idiopathic Baseline Latent Space (PD vs. HC)", 
            "pca_3d_track1_pd_vs_hc", palette
        )
        
        # --- LOOP SEQUENCE 2: PD vs Differential Diagnosis ---
        generate_3d_spin_sequence(
            X_scaled, y, ["PD", "DD"], 
            "Track 2: Phenotypic Mimicry Latent Space (PD vs. DD)", 
            "pca_3d_track2_pd_vs_dd", palette
        )

        print("\nAll systems clear. All 72 rotation images successfully generated in the 'figs/' directory!")
