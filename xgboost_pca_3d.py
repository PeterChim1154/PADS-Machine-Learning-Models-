...
print("Launching Corrected Patient-Isolated Dual-Track Architecture with 3D PCA...")
...
# =====================================================================
# 3. RUN COHORT DISCRIMINATION ENGINE (WITH 3D PCA EMBEDDED)
# =====================================================================
def run_optimized_track(X_all, y_all, groups_all, target_label, control_label):
   ...
   # -------------------------------------------------------------
   # GENERATE 3D PCA EMBEDDING & PLOT EXPORT
   # -------------------------------------------------------------
   ...
   ax.set_title(f"XGBoost Context - 3D PCA Variance Space ({control_label} vs {target_label})\nCaptured Global Variance: {explained_var:.1f}%")
   ax.set_xlabel("PC 1")
   ax.set_ylabel("PC 2")
   ax.set_zlabel("PC 3")
   ...
   plt.tight_layout()
   filename_clean = f"xgb_pca_3d_{control_label.lower().replace(' ', '_')}_vs_{target_label.lower().replace(' ', '_')}.png"
   plt.savefig(filename_clean, dpi=300)
   plt.close()
   print(f" -> 3D Plot successfully exported to disk: '{filename_clean}'")
   # -------------------------------------------------------------
   ...
   for train_idx, test_idx in sgkf.split(X_tr, y_bin, groups_tr):
       ...
       # Train on 3 dimensions instead of 2
       ...
       model.fit(X_train_pca, y_train, sample_weight=w)
   ...
   print("\n" + "="*50)
   print(f"BALANCED RESULTS TRACK: [{control_label}] vs [{target_label}] (3D PCA DATA)")
