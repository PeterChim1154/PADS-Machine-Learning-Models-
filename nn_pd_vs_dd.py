...
class PDvsDDNeuralNetwork(nn.Module):
    def __init__(self, input_dim):
        super(PDvsDDNeuralNetwork, self).__init__()
...
def train_and_evaluate_nn(X, y, groups, modality_name):
    ...
    for train_idx, test_idx in sgkf.split(X_mat, y_mat, groups=groups_mat):
        ...
        model = PDvsDDNeuralNetwork(input_dim=X_train.shape[1])
...
try:
    ...
    # Filter STRICTLY for Parkinson's vs Differential Diagnosis conditions
    dd_conditions = ['MULTIPLE SCLEROSIS', 'OTHER MOVEMENT DISORDERS', 'ESSENTIAL TREMOR', 'ATYPICAL PARKINSONISM']
    task_df = df[df['condition'].isin(["PARKINSON'S"] + dd_conditions)].copy()
    
    # Standardize all alternative disorders to 'DD' label
    task_df['condition'] = task_df['condition'].apply(lambda x: 'DD' if x in dd_conditions else x)
    ...
    print(f"\n==============================================================================================")
    print(f" SEPARATE TASK: Parkinson's (PD) vs. Differential Diagnosis (DD) (FEED-FORWARD NEURAL NETWORK)")
    print(f" Samples: {task_df.shape[0]} | Patients: {groups.nunique()} | (DD=0, PARKINSON'S=1)")
    print(f"==============================================================================================")
