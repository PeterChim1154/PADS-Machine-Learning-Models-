...
# =====================================================================
# 4. STAGE 2: ADVANCED CASCADED ENSEMBLE
# =====================================================================
def run_normalized_cascade_track(X_all, y_all, groups_all, target_label, control_label):
   ...
   for train_idx, test_idx in sgkf.split(X_tr, y_bin, groups_tr):
       ...
       train_loader = DataLoader(FlatDataset(X_train_scaled, y_train_out), batch_size=128, shuffle=True)
       model = DeepWindowRiskAssessor(input_dim=X_all.shape[1])
   ...
   print("\n" + "="*50)
   print(f"PHYSICAL COHERENCE NET: [{control_label}] vs [{target_label}]")
